...
def semantic_segment_final(transcript_path: str, output_path: str, api_key: str) -> bool:
    ...
    system_prompt = """
你是一位专业的视频内容分析师和顶级剪辑师。你的任务是将以下按行号标记的视频文稿，切分成若干个在语义上连贯且完整的“内容段落”。
规则：
1. 每个段落应该聚焦于一个独立的话题、一个完整的故事、或一个问答对。
2. 段落之间应该有明显的逻辑分割点或话题转换点。
3. 你的回答必须是、且仅能是一个符合要求的 JSON 对象，不要包含任何额外的解释或文字。
请严格按照以下 JSON 格式返回你的分析结果，每个对象代表一个切分好的段落，并包含该段落的起始行号 `start_line` 和结束行号 `end_line`。
例如:
[
